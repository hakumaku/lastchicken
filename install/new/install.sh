...
POWERLINE_CONFIG_PATH="$HOME/.config/powerline"
BASHRC="$HOME/.bashrc"
BASHRC_CONFIG="$DIR/bashrc.dat"
...
echo -e "✔"
# if [[ ! -f $VIMRC ]]; then
# 	echo -e "\n${CYAN}.vimrc${NC} file does not exist in ${LIGHT_BLUE}$VIMRC${NC}"
# 	exit 1
# fi
...
# ~/.bashrc commands to run powerline-status.
# while IFS='' read -r line; do
...
# 	echo -e "\n${CYAN}config.json${NC} file does not exist in ${LIGHT_BLUE}$POWERLINE_CONFIG${NC}"
# 	exit 1
# fi
# mkdir $POWERLINE_CONFIG_PATH && cp $POWERLINE_CONFIG $POWERLINE_CONFIG_PATH
echo -e "✔"
...
# 	echo -e "\n${CYAN}.tmux.conf${NC} file does not exist at ${LIGHT_BLUE}$DIR${NC}"
# 	exit 1
# fi
# cp $TMUX_CONFIG $HOME
echo -e "✔\n"
